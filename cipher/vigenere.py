...
msg = ("1. Decide if this message is to be encrypted or decrypted.\n2. Open a txt file with the message you want to alter. Once selected, the text should appear in the 'Original Text' box. \n3. Enter a keyword and click the 'code' button\n4. If encypting, the encrypted message will appear in the box on the right. If decrypting, a readable message should appear in the box on the right. \n5. If you want, you can save the output of the box on the right with the 'Save As' button")
...
class App(Frame):
    def __init__(self, root):
        #establish frames
        Frame.__init__(self,root, pady=65, bg="grey")
        topframe = Frame(root, pady=5, bg="grey")
        topframe.pack(side=TOP,fill=X)
        leftframe = Frame(root, bg="grey")
        leftframe.pack(side=LEFT,padx=10, fill=Y)
        rightframe2 = Frame(root, bg="grey")
        rightframe2.pack(side=RIGHT)
        rightframe = Frame(root, bg="grey")
        rightframe.pack(padx=3,side=RIGHT)
        radioframe = Frame(leftframe, bg="grey")
        radioframe.pack(pady=100)
        helpframe = Frame(leftframe, bg="grey")
        helpframe.pack(side=BOTTOM)
        
        #make directory button
        button_opt = {'fill': Tkconstants.BOTH, 'padx': 5, 'pady': 5}
        Button(self, text='open file',highlightbackground="grey", command=self.askopenfile).pack(**button_opt)
        buttonlabel = Label(self, text = "enter keyword", bg="grey")
        buttonlabel.pack()
        self.entryfield = Entry(self,highlightbackground="grey")
        self.entryfield.pack()
        Button(self, text='code', command=self.code,highlightbackground="grey").pack(**button_opt)
        Button(self, text='Save As', command=self.asksaveasfile,highlightbackground="grey").pack(**button_opt)
        ...
        self.var = IntVar()
        R1 = Radiobutton(radioframe, text="encrypt", variable=self.var, value=1, command=sel, bg="grey")
        R1.pack(side=TOP)
        R2 = Radiobutton(radioframe, text="decrypt", variable=self.var, value=2,comman=sel, bg="grey")
        R2.pack(side=TOP)
        R1.select()
        ...
        originallabel = Label(rightframe, text="Original Text", bg="grey")
        ...
        alteredlabel = Label(rightframe2, bg="grey")
        ...
        #help button
        Button(helpframe, text='help', command=self.helpinfo,highlightbackground="grey").pack(**button_opt)
    
    def askopenfile(self):
        file = tkFileDialog.askopenfile(mode='r', **self.file_opt)
        #print file, words, "yolololol"
    ...
    def code(self):
        if not words:
            tkMessageBox.showerror(title="error",message="Open a file first")
        else:
            self.text2.delete(1.0, END)
